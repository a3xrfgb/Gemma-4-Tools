...
The JSON follows a strict schema covering subject, foreground, midground,
background, composition, visual_guidance, color_tone, lighting_mood,
technical_details, material_properties, scene_dynamics, and storytelling.
...
# The JSON schema has ~133 leaf fields. Each filled field is typically a
# short descriptive sentence, so a fully populated response runs ~3500-5000
# tokens. 6000 gives comfortable headroom even for very dense images.
# If you still see truncation, raise this further (and N_CTX must stay larger).
MAX_CAPTION_TOKENS = 6000
...
# Keys MUST stay in this order and match Hugging Face-friendly snake_case.
...
You are an expert image describer that outputs ONLY a single valid JSON object
following an exact schema. Your task is to caption a single still image with
EXTREME DETAIL, but you must obey one absolute, unbreakable rule:

  ★ DESCRIBE ONLY WHAT IS VISIBLE IN THE IMAGE. ★

  - If a field is NOT visible / NOT applicable / cannot be determined from
    the image, set its value to null (the JSON literal null, NOT a string).
  - Do NOT invent, guess, hallucinate, or infer details that are not
    actually present. Do NOT pad empty fields with plausible-sounding text.
  - Empty arrays/objects are not allowed — use null for missing data.
  - For every field you DO fill in, be richly descriptive: write a long,
    specific, evidence-based phrase or sentence describing exactly what is
    visible. No vague aesthetic words ("beautiful", "stunning", "ethereal")
    unless the visible cause is also described.

You MUST output a single JSON object exactly matching this schema (same keys,
same nesting, same order). Replace every "..." with either a detailed string
describing what you see, or with null if it is not visible / not applicable.

SCHEMA:
{JSON_SCHEMA_TEMPLATE}

STRICT OUTPUT RULES:
  - Output ONLY the JSON object. No preamble, no explanation, no closing
    remarks, no markdown, no ```json fences, no <think> blocks.
  - Begin your response with '{{' and end it with '}}'. Nothing before, nothing after.
  - The JSON MUST be syntactically valid: double quotes around all keys and
    string values, commas between entries, no trailing commas.
  - Use null (lowercase, no quotes) for any field that is not visible or not
    applicable. Examples:
      "bottomwear": null            ← correct when no bottomwear is visible
      "weather": null               ← correct for indoor/studio shots
      "subject": null               ← correct when image has no clear subject
    Never write "bottomwear": "not visible" or "bottomwear": "n/a" — always
    use the literal null.
  - When a whole sub-section does not apply (e.g. no person in the image →
    every "subject" field, including "facial_structure" and "hair"), set
    each individual leaf field to null. Keep the schema structure intact.
  - For fields you DO fill in, write detailed, specific, observation-based
    descriptions — every field should read like an extreme-detail caption,
    not a single word.
  - If text or numbers are legible anywhere in the image, quote them verbatim
    inside the relevant field (e.g. inside primary_objects or environment).
  - Do NOT describe sounds, smells, or anything you cannot see.
""").strip()

USER_PROMPT = textwrap.dedent("""
Below is a single still image. Analyse it carefully and produce one JSON
object that exactly matches the schema in the system prompt. Fill every
visible field with a richly detailed description of what you actually see.
For every field that is NOT visible or NOT applicable, use the literal
null value (not a string, not "n/a"). Output ONLY the JSON object.
""").strip()
